...
import { computeBalancerState } from '@/lib/engine/loadBalancer';
...
export async function GET(
  request: NextRequest
): Promise<NextResponse<BalancerState>>
{
  const searchParams = request.nextUrl.searchParams;
  const algorithm = (
    searchParams.get('algorithm') || 'round-robin'
  ) as BalancerAlgorithm;
  const classified = buildClassifiedNodes();
  const topology = buildTopology(classified);
  const state = computeBalancerState(topology.nodes, algorithm, []);
