...
import { generateTrafficBatch, computeTrafficFlow } from '@/lib/engine/trafficRouter';
...
export async function GET(
  request: NextRequest
): Promise<NextResponse<TrafficFlowData>>
{
  const searchParams = request.nextUrl.searchParams;
  const algorithm = (searchParams.get('algorithm') || 'round-robin') as BalancerAlgorithm;
  ...
  const packets = generateTrafficBatch(topology.nodes, algorithm, count);
  const flow = computeTrafficFlow(packets, topology.nodes);
